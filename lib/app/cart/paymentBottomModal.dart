import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:animations/animations.dart';

import '../../providers/index.dart' show CartProvider;
import '../../util/index.dart' show Constants;

class PaymentBottomModal extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Consumer<CartProvider>(
      builder: (context, provider, child) {
        return provider.numberOfItemsInCart > 0
            ? _payment(context, provider)
            : Container();
      },
    );
  }

  Widget _payment(BuildContext context, CartProvider provider) {
    return Material(
      elevation: 3.0,
      child: Container(
        height: 50,
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            Expanded(
              child: Container(
                color: Theme.of(context).cardColor,
                child: Center(
                  child: Text(
                    'Time for payment',
                    style: TextStyle(
                      fontSize: 17,
                      fontWeight: FontWeight.bold,
                    ),
                  ),
                ),
              ),
            ),
            Expanded(
              child: _paymentButton(context, provider),
            )
          ],
        ),
      ),
    );
  }

  Widget _paymentButton(BuildContext context, CartProvider provider) {
    final _themeData = Theme.of(context);

    return OpenContainer(
      closedColor: _themeData.primaryColor,
      transitionDuration: Constants.kContainerAnimationDuration,
      closedBuilder: (context, openContainer) {
        return GestureDetector(
          onTap: openContainer,
          child: Container(
            color: _themeData.primaryColor,
            child: Center(
              child: Icon(
                Icons.payment,
                color: _themeData.cardColor,
                size: 35,
              ),
            ),
          ),
        );
      },
      openBuilder: (_, __) => Scaffold(
        appBar: AppBar(
          title: Text('Payment'),
        ),
      ),
    );
  }
}
